...
namespace HD3_ServerEmployee
{
    public class Employee
    {
        ...
        public byte[] GetBytes()
        {
            ...
            place += 4;
            Buffer.BlockCopy(BitConverter.GetBytes(LastName.Length), 0, data, place, 4); // FIXED
            place += 4;
            Buffer.BlockCopy(Encoding.ASCII.GetBytes(LastName), 0, data, place, LastName.Length);
            place += LastName.Length;
            Buffer.BlockCopy(BitConverter.GetBytes(FirstName.Length), 0, data, place, 4); // FIXED
            place += 4;
